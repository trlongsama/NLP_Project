...
class FeatureExtractor(object):
    ...
    def get_input_representation(self, words, pos, state):
        stack, buffer, deps = state.stack, state.buffer, state.deps
        stack_len, buff_len = min(len(stack),3), min(len(buffer),3)
        stack_idx = [stack[-(i+1)] for i in range(stack_len)]+['NULL']*(3-stack_len)
        buff_idx = [buffer[-(i + 1)] for i in range(buff_len)] + ['NULL'] * (3 - buff_len)
        idx_lst = stack_idx+buff_idx
        input_lst = []
        for item in idx_lst:
            if item == 'NULL':
                input_lst.append(self.word_vocab['<NULL>'])
            elif item == 0:
                input_lst.append(self.word_vocab['<ROOT>'])
            elif words[item] not in self.word_vocab:
                if pos[item] == 'CD':
                    input_lst.append(self.word_vocab['<CD>'])
                elif pos[item] == 'NNP':
                    input_lst.append(self.word_vocab['<NNP>'])
                else:
                    input_lst.append(self.word_vocab['<UNK>'])
            else:
                input_lst.append(self.word_vocab[words[item]])

        return np.array(input_lst)

    def get_output_representation(self, output_pair):
        idx = self.output_labels[output_pair]
        res = [1 if i == idx else 0 for i in range(91)]
        return np.array(res)
